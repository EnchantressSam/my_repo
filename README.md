...
Мы можем использовать её, чтобы обратиться к элементу напрямую:
...
<script>
  ...
  alert(window.elem); // такой доступ к глобальной переменной также работает

  // для `elem-content` всё немного сложнее,
  // так как в имени переменной дефис `-`
  alert(window['elem-content']); // ... поэтому доступно через квадратные скобки [...]
</script>
```
...
Иначе говоря, результат такой же, как при `elem.querySelectorAll(css)[0]`, но в последнем вызове сначала ищутся *все* элементы, а потом берётся первый, а `elem.querySelector` ищется только первый. Так писать быстрее и короче.
...
Буква `"s"` отсутствует в названии метода `getElementById`, так как возвращает один элемент. Но `getElementsByTagName` вернёт список элементов, поэтому `"s"` обязательна.
```
...
Попытка присвоить значение *коллекции*, а не элементам внутри неё.
...
1. Первый создает ссылку на коллекцию `<div>`. На данный момент её длина равна `1`.
...
Мы использовали отдельные скрипты, чтобы увидеть как добавление элемента влияет на коллекцию, но любые манипуляции с DOM влияют на них. Скоро мы увидим их больше.
